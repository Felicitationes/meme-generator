...
export function initMemeEditor(container) {
  const canvas = container.querySelector('#memeCanvas');
  ...
  const topTextInput = container.querySelector('#topText');
  const bottomTextInput = container.querySelector('#bottomText');
  const fontSizeInput = container.querySelector('#fontSize');
  const fontSizeValue = container.querySelector('#fontSizeValue');
  const textColorInput = container.querySelector('#textColor');
  const downloadBtn = container.querySelector('#downloadBtn');
  const changeImageBtn = container.querySelector('#changeImageBtn');
  const templatesSection = container.querySelector('#templatesSection');
  const uploadDivider = container.querySelector('#uploadDivider');
  ...
  function loadImageFromUrl(url) {
    const img = new Image();
    img.onload = () => loadImageFromSrc(img.src);
    img.onerror = () => console.error('Failed to load template image:', url);
    img.src = url;
  }
  ...
  function draw() {
    ...
    if (topText) drawTextBlock(topText, topPos.x, topPos.y, lineHeight, fillColor);
    if (bottomText) drawTextBlock(bottomText, bottomPos.x, bottomPos.y, lineHeight, fillColor);
  }
  ...
  canvas.addEventListener('mousemove', (e) => {
    ...
    if (draggingText) {
      ...
      if (draggingText === 'top') topTextPos = { x: newX, y: newY };
      else bottomTextPos = { x: newX, y: newY };
      draw();
    } else {
